package com.cityinspector.premierletter.action;
...
/**
 * @author cuiyp
 * 创建时间：2018-07-20 11:14:47
 */
@SuppressWarnings("finally")
@Controller
public class PremierLetterAction extends BaseAction implements ModelDriven<Object>{
	 ...
	 * 将form转成model
	 */
	private PremierLetter premierLetter;
	public PremierLetter getModel() {
		if(premierLetter == null) {
			premierLetter = new PremierLetter();
		}
	...
	public String addPremierLetter() throws Exception {
		return "edit";
	}
	...
	public String detailPremierLetter() throws Exception {
		String id = request.getParameter("id");
		premierLetter = premierLetterService.getPremierLetterById(id);
		request.setAttribute("premierLetter", premierLetter);
		return "detail";
	}
	...
	public String editPremierLetter() throws Exception {
		String id = request.getParameter("id");
		premierLetter = premierLetterService.getPremierLetterById(id);
		request.setAttribute("premierLetter", premierLetter);
		return "edit";
	...
	public String delPremierLetterById() throws IOException {
		String id = request.getParameter("id");
		try {
			premierLetter.setId(id);
			premierLetterService.delPremierLetter(premierLetter);
		} catch (Exception e) {
	...
	public String delPremierLetterByIds() throws IOException {
		...
		try {
			...
			premierLetterService.delPremierLetterBatch(idList);
		} catch (Exception e) {
	...
	public String savePremierLetter() throws Exception {
		premierLetterService.savePremierLetter(premierLetter);
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/premierletter/searchPremierLetter.do");
		return "success";
	}
	...
	public String searchPremierLetter() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("premierLetterList").		//premierLetterList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(!StringUtil.null2String(premierLetter.getTitle()).equals("")) {
				sqlWhere.append(" and premierLetter.title='" + premierLetter.getTitle() + "'");
			}
			if(!StringUtil.null2String(premierLetter.getReqregisteruser()).equals("")) {
				sqlWhere.append(" and premierLetter.reqregisteruser='" + premierLetter.getReqregisteruser() + "'");
			}
			if(!StringUtil.null2String(premierLetter.getRepstatus()).equals("")) {
				sqlWhere.append(" and premierLetter.repstatus='" + premierLetter.getRepstatus() + "'");
			}
			request.setAttribute("premierLetter", premierLetter);
		} else {
			request.setAttribute("premierLetter", null);
		}
		
		sqlWhere.append(" order by reqtime desc");
		...
		Map map = premierLetterService.searchPremierLetter(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openPremierLetterExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = premierLetter.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and premierLetter.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<PremierLetter> list = premierLetterService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(PremierLetter premierLetter : list) {
				Object[] tempModel = new Object[ja.size()];									//临时的model
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = premierLetter.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(premierLetter, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openPremierLetterImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, PremierLetter.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<PremierLetter> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					premierLetterService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/premierletter/searchPremierLetter.do");
	 ...
	 */
	@Security
	public String savePremierLetter4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 		premierLetter = premierLetterService.savePremierLetter(premierLetter);
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(premierLetter).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editPremierLetter4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delPremierLetterByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		premierLetterService.delPremierLetterBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchPremierLetter4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
