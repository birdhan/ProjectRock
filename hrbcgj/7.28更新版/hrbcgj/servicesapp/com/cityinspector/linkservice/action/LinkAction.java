package com.cityinspector.linkservice.action;
...
/**
 * @author 王富生
 * 创建时间：2018-07-21 09:15:29
 */
@SuppressWarnings("finally")
@Controller
public class LinkAction extends BaseAction implements ModelDriven<Object>{
	...
	public String detailLink() throws Exception {
	...
	public String delLinkById() throws IOException {
		String id = request.getParameter("id");
		try {
			link.setId(id);
	...
	public String delLinkByIds() throws IOException {
		...
		try {
			...
			linkService.delLinkBatch(idList);
		} catch (Exception e) {
	...
	public String saveLink() throws Exception {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/linkservice/searchLink.do");
		return "success";
	}
	...
	public String searchLink() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("linkList").		//linkList为jsp页面中<display:table> 的id值
		...
		if(isSearch.equals("yes")) {													//表示是查询
			...
			if(!StringUtil.null2String(link.getLinkName()).equals("")) {
				sqlWhere.append(" and link.linkName='" + link.getLinkName() + "'");
			}
			request.setAttribute("link", link);
		} else {
		...
		Map map = linkService.searchLink(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openLinkExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = link.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and link.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<Link> list = linkService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(Link link : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = link.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(link, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openLinkImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, Link.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<Link> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					linkService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/linkservice/searchLink.do");
	 ...
	 */
	@Security
	public String saveLink4Json() throws Exception {
	 	...
	 	try {
	 		...
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(link).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editLink4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delLinkByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		linkService.delLinkBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchLink4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
