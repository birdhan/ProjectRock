package com.cityinspector.topic.action;
...
/**
 * @author 王富生
 * 创建时间：2018-07-22 13:03:23
 */
@SuppressWarnings("finally")
@Controller
public class TopicAction extends BaseAction implements ModelDriven<Object>{
	...
	public String detailTopic() throws Exception {
	...
	public String delTopicById() throws IOException {
		String id = request.getParameter("id");
		try {
			topic.setId(id);
	...
	public String delTopicByIds() throws IOException {
		...
		try {
			...
			topicService.delTopicBatch(idList);
		} catch (Exception e) {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/topic/searchTopic.do");
		return "success";
	}
	...
	public String searchTopic() throws Exception {
		...
		String pageIndexName = new org.displaytag.util.ParamEncoder("topicList").		//topicList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {												//表示是查询
			if(!StringUtil.null2String(topic.getTitle()).equals("")) {
				sqlWhere.append(" and topic.title='" + topic.getTitle() + "'");
			}
			request.setAttribute("topic", topic);
		} else {
			request.setAttribute("topic", null);
		}
		...
		Map map = topicService.searchTopic(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openTopicExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = topic.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and topic.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<Topic> list = topicService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(Topic topic : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = topic.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(topic, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openTopicImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, Topic.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<Topic> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					topicService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/topic/searchTopic.do");
	 ...
	 */
	@Security
	public String saveTopic4Json() throws Exception {
	 	...
	 	try {
	 		...
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(topic).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editTopic4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delTopicByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		topicService.delTopicBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchTopic4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
