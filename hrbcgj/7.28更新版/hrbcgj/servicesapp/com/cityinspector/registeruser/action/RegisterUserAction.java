package com.cityinspector.registeruser.action;
...
/**
 * @author cuiyp
 * 创建时间：2018-07-18 18:00:12
 */
@SuppressWarnings("finally")
@Controller
public class RegisterUserAction extends BaseAction implements ModelDriven<Object>{
	...
	public String addRegisterUser() throws Exception {
		return "edit";
	}
	...
	public String detailRegisterUser() throws Exception {
		String id = request.getParameter("id");
		registerUser = registerUserService.getRegisterUserById(id);
	...
	public String editRegisterUser() throws Exception {
		String id = request.getParameter("id");
		registerUser = registerUserService.getRegisterUserById(id);
		request.setAttribute("registerUser", registerUser);
		return "edit";
	...
	public String delRegisterUserById() throws IOException {
		String id = request.getParameter("id");
		try {
			registerUser.setId(id);
			registerUserService.delRegisterUser(registerUser);
		} catch (Exception e) {
	...
	public String delRegisterUserByIds() throws IOException {
		...
		try {
			...
			registerUserService.delRegisterUserBatch(idList);
		} catch (Exception e) {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/registeruser/searchRegisterUser.do");
		return "success";
	}
	...
	public String searchRegisterUser() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("registerUserList").		//registerUserList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(!StringUtil.null2String(registerUser.getAccount()).equals("")) {
				sqlWhere.append(" and registerUser.account='" + registerUser.getAccount() + "'");
			}
			if(!StringUtil.null2String(registerUser.getUsername()).equals("")) {
				sqlWhere.append(" and registerUser.username='" + registerUser.getUsername() + "'");
			}
			request.setAttribute("registerUser", registerUser);
		} else {
		...
		Map map = registerUserService.searchRegisterUser(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openRegisterUserExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = registerUser.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and registerUser.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<RegisterUser> list = registerUserService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(RegisterUser registerUser : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = registerUser.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(registerUser, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openRegisterUserImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, RegisterUser.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<RegisterUser> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					registerUserService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/registeruser/searchRegisterUser.do");
	 ...
	 */
	@Security
	public String saveRegisterUser4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 		registerUser = registerUserService.saveRegisterUser(registerUser);
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(registerUser).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editRegisterUser4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delRegisterUserByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		registerUserService.delRegisterUserBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchRegisterUser4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
