package com.cityinspector.guide.action;
...
import com.cityinspector.guide.service.IGuideService;
import com.opensymphony.xwork2.ModelDriven;

/**
 * @author cuiyp
 * 创建时间：2018-07-16 14:18:54
 */
@SuppressWarnings("finally")
@Controller
public class GuideAction extends BaseAction implements ModelDriven<Object>{
	...
	public String detailGuide() throws Exception {
	...
	public String editGuide() throws Exception {
		String id = request.getParameter("id");
		guide = guideService.getGuideById(id);
		request.setAttribute("guide", guide);
		return "edit";
	...
	public String delGuideById() throws IOException {
		String id = request.getParameter("id");
		try {
			guide.setId(id);
			guideService.delGuide(guide);
		} catch (Exception e) {
	...
	public String delGuideByIds() throws IOException {
		...
		try {
			...
			guideService.delGuideBatch(idList);
		} catch (Exception e) {
	...
	public String saveGuide() throws Exception {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/guide/searchGuide.do");
		return "success";
	}
	...
	public String searchGuide() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("guideList").		//guideList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(!StringUtil.null2String(guide.getTitle()).equals("")) {
				sqlWhere.append(" and guide.title='" + guide.getTitle() + "'");
			}
			request.setAttribute("guide", guide);
		} else {
			request.setAttribute("guide", null);
		}
		...
		Map map = guideService.searchGuide(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openGuideExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = guide.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and guide.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<Guide> list = guideService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(Guide guide : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = guide.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(guide, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openGuideImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, Guide.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<Guide> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					guideService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/guide/searchGuide.do");
	 ...
	 */
	@Security
	public String saveGuide4Json() throws Exception {
	 	...
	 	try {
	 		...
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(guide).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editGuide4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delGuideByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		guideService.delGuideBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchGuide4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
