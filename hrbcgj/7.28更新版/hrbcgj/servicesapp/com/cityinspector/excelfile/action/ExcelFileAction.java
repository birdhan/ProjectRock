package com.cityinspector.excelfile.action;
...
import com.cityinspector.excelfile.service.IExcelFileService;
import com.opensymphony.xwork2.ModelDriven;

/**
 * @author cuiyp
 * 创建时间：2018-07-16 10:18:39
 */
@SuppressWarnings("finally")
@Controller
public class ExcelFileAction extends BaseAction implements ModelDriven<Object>{
	...
	public String addExcelFile() throws Exception {
		return "edit";
	}
	...
	public String detailExcelFile() throws Exception {
		String id = request.getParameter("id");
		excelFile = excelFileService.getExcelFileById(id);
		request.setAttribute("excelFile", excelFile);
		return "detail";
	}
	...
	public String editExcelFile() throws Exception {
		String id = request.getParameter("id");
		excelFile = excelFileService.getExcelFileById(id);
		request.setAttribute("excelFile", excelFile);
		return "edit";
	...
	public String delExcelFileById() throws IOException {
		String id = request.getParameter("id");
		try {
			excelFile.setId(id);
			excelFileService.delExcelFile(excelFile);
		} catch (Exception e) {
	...
	public String delExcelFileByIds() throws IOException {
		...
		try {
			...
			excelFileService.delExcelFileBatch(idList);
		} catch (Exception e) {
	...
	public String saveExcelFile() throws Exception {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/excelfile/searchExcelFile.do");
		return "success";
	}
	...
	public String searchExcelFile() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("excelFileList").		//excelFileList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(!StringUtil.null2String(excelFile.getName()).equals("")) {
				sqlWhere.append(" and excelFile.name='" + excelFile.getName() + "'");
			}
			request.setAttribute("excelFile", excelFile);
		} else {
			request.setAttribute("excelFile", null);
		}
		...
		Map map = excelFileService.searchExcelFile(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openExcelFileExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = excelFile.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and excelFile.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<ExcelFile> list = excelFileService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(ExcelFile excelFile : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = excelFile.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(excelFile, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openExcelFileImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, ExcelFile.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<ExcelFile> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					excelFileService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/excelfile/searchExcelFile.do");
	 ...
	 */
	@Security
	public String saveExcelFile4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 		excelFile = excelFileService.saveExcelFile(excelFile);
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(excelFile).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editExcelFile4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delExcelFileByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		excelFileService.delExcelFileBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchExcelFile4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
