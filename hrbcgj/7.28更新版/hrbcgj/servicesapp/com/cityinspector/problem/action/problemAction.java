package com.cityinspector.problem.action;
...
/**
 * @author wst
 * 创建时间：2018-07-22 14:23:21
 */
@SuppressWarnings("finally")
@Controller
public class problemAction extends BaseAction implements ModelDriven<Object>{
	...
	public String detailproblem() throws Exception {
	...
	public String editproblem() throws Exception {
		String id = request.getParameter("id");
		problem = problemService.getproblemById(id);
		request.setAttribute("problem", problem);
		return "edit";
	...
	public String delproblemById() throws IOException {
		String id = request.getParameter("id");
		try {
			problem.setId(id);
			problemService.delproblem(problem);
		} catch (Exception e) {
	...
	public String delproblemByIds() throws IOException {
		...
		try {
			...
			problemService.delproblemBatch(idList);
		} catch (Exception e) {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/problem/searchproblem.do");
		return "success";
	}
	...
	public String searchproblem() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("problemList").		//problemList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {	
			if(!StringUtil.null2String(problem.getTitle()).equals("")) {
				sqlWhere.append(" and problem.title='" + problem.getTitle() + "'");
			}
			/*if(!StringUtil.null2String(problem.getUserName()).equals("")) {
				sqlWhere.append(" and problem.userName='" + problem.getUserName() + "'");
			}
			
			if(!StringUtil.null2String(problem.getStatus()).equals("")) {
				
				sqlWhere.append(" and problem.status='" + problem.getStatus() + "'");
			}*///表示是查询
			request.setAttribute("problem", problem);
		} else {
		...
		Map map = problemService.searchproblem(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openproblemExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = problem.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and problem.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<problem> list = problemService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(problem problem : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = problem.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(problem, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openproblemImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, problem.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<problem> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					problemService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/problem/searchproblem.do");
	 ...
	 */
	@Security
	public String saveproblem4Json() throws Exception {
	 	...
	 	try {
	 		...
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(problem).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editproblem4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delproblemByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		problemService.delproblemBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchproblem4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
