package com.cityinspector.onlineservice.action;
...
import com.cityinspector.onlineservice.service.IOnlineServiceService;
import com.opensymphony.xwork2.ModelDriven;

/**
 * @author cuiyp
 * 创建时间：2018-07-16 14:36:44
 */
@SuppressWarnings("finally")
@Controller
public class OnlineServiceAction extends BaseAction implements ModelDriven<Object>{
	...
	public String addOnlineService() throws Exception {
		return "edit";
	}
	...
	public String detailOnlineService() throws Exception {
		String id = request.getParameter("id");
		onlineService = onlineServiceService.getOnlineServiceById(id);
		request.setAttribute("onlineService", onlineService);
		return "detail";
	}
	...
	public String editOnlineService() throws Exception {
		String id = request.getParameter("id");
		onlineService = onlineServiceService.getOnlineServiceById(id);
		request.setAttribute("onlineService", onlineService);
		return "edit";
	...
	public String delOnlineServiceById() throws IOException {
		String id = request.getParameter("id");
		try {
			onlineService.setId(id);
			onlineServiceService.delOnlineService(onlineService);
		} catch (Exception e) {
	...
	public String delOnlineServiceByIds() throws IOException {
		...
		try {
			...
			onlineServiceService.delOnlineServiceBatch(idList);
		} catch (Exception e) {
	...
	public String saveOnlineService() throws Exception {
		onlineServiceService.saveOnlineService(onlineService);
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/onlineservice/searchOnlineService.do");
		return "success";
	}
	...
	public String searchOnlineService() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("onlineServiceList").		//onlineServiceList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(!StringUtil.null2String(onlineService.getTitle()).equals("")) {
				sqlWhere.append(" and onlineService.title='" + onlineService.getTitle() + "'");
			}
			request.setAttribute("onlineService", onlineService);
		} else {
			request.setAttribute("onlineService", null);
		}
		...
		Map map = onlineServiceService.searchOnlineService(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openOnlineServiceExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = onlineService.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and onlineService.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<OnlineService> list = onlineServiceService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(OnlineService onlineService : list) {
				Object[] tempModel = new Object[ja.size()];									//临时的model
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = onlineService.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(onlineService, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openOnlineServiceImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, OnlineService.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<OnlineService> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					onlineServiceService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/onlineservice/searchOnlineService.do");
	 ...
	 */
	@Security
	public String saveOnlineService4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 		onlineService = onlineServiceService.saveOnlineService(onlineService);
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(onlineService).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editOnlineService4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delOnlineServiceByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		onlineServiceService.delOnlineServiceBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchOnlineService4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
