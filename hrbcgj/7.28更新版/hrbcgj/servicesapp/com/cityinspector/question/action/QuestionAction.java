package com.cityinspector.question.action;
...
import com.cityinspector.question.service.IQuestionService;
import com.opensymphony.xwork2.ModelDriven;

/**
 * @author cuiyp
 * 创建时间：2018-07-16 13:53:15
 */
...
public class QuestionAction extends BaseAction implements ModelDriven<Object>{
	...
	public String detailQuestion() throws Exception {
	...
	public String delQuestionById() throws IOException {
		String id = request.getParameter("id");
		try {
			question.setId(id);
	...
	public String delQuestionByIds() throws IOException {
		...
		try {
			...
			questionService.delQuestionBatch(idList);
		} catch (Exception e) {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/question/searchQuestion.do");
		return "success";
	}
	...
	public String searchQuestion() throws Exception {
		...
		String pageIndexName = new org.displaytag.util.ParamEncoder("questionList").		//questionList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(!StringUtil.null2String(question.getName()).equals("")) {
				sqlWhere.append(" and question.name='" + question.getName() + "'");
			}
			request.setAttribute("question", question);
		} else {
		...
		Map map = questionService.searchQuestion(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openQuestionExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = question.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and question.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<Question> list = questionService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(Question question : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = question.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(question, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openQuestionImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, Question.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<Question> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					questionService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/question/searchQuestion.do");
	 ...
	 */
	@Security
	public String saveQuestion4Json() throws Exception {
	 	...
	 	try {
	 		...
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(question).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editQuestion4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delQuestionByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		questionService.delQuestionBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchQuestion4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
