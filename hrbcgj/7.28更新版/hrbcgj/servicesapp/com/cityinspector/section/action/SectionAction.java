package com.cityinspector.section.action;
...
/**
 * @author cuiyp
 * 创建时间：2018-07-15 20:24:34
 */
@SuppressWarnings("finally")
@Controller
public class SectionAction extends BaseAction implements ModelDriven<Object>{
	...
	public String detailSection() throws Exception {
	...
	public String editSection() throws Exception {
		...
		List<Section> sectionList = sectionService.getAllDataByWhere(" and id !='" + id + "'");
		request.setAttribute("sectionList", sectionList);
	...
	public String delSectionById() throws IOException {
		String id = request.getParameter("id");
		try {
			section.setId(id);
			sectionService.delSection(section);
		} catch (Exception e) {
	...
	public String delSectionByIds() throws IOException {
		...
		try {
			...
			sectionService.delSectionBatch(idList);
		} catch (Exception e) {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/section/searchSection.do");
		return "success";
	}
	...
	public String searchSection() throws Exception {
		PAGESIZE_CONSTANT = 20l;
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("sectionList").		//sectionList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(!StringUtil.null2String(section.getName()).equals("")) {
				sqlWhere.append(" and section.name='" + section.getName() + "'");
			}
			if(!StringUtil.null2String(section.getPostion()).equals("")) {
				sqlWhere.append(" and section.postion='" + section.getPostion() + "'");
			}
			if(!StringUtil.null2String(section.getIsshow()).equals("")) {
				sqlWhere.append(" and section.isshow='" + section.getIsshow() + "'");
			}
			if(!StringUtil.null2String(section.getPid()).equals("")) {
				sqlWhere.append(" and section.pid='" + section.getPid() + "'");
			}
			request.setAttribute("section", section);
		} else {
		...
		sqlWhere.append(" order by postion, sortnum");
		...
		Map map = sectionService.searchSection(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openSectionExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = section.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and section.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<Section> list = sectionService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(Section section : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = section.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(section, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openSectionImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, Section.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<Section> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					sectionService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/section/searchSection.do");
	 ...
	 */
	@Security
	public String saveSection4Json() throws Exception {
	 	...
	 	try {
	 		...
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(section).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editSection4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delSectionByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		sectionService.delSectionBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchSection4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
