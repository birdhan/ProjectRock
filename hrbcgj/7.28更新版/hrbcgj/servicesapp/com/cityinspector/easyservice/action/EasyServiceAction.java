package com.cityinspector.easyservice.action;
...
/**
 * @author cuiyp
 * 创建时间：2018-07-16 17:52:26
 */
@SuppressWarnings("finally")
@Controller
public class EasyServiceAction extends BaseAction implements ModelDriven<Object>{
	...
	public String addEasyService() throws Exception {
		return "edit";
	}
	...
	public String detailEasyService() throws Exception {
		String id = request.getParameter("id");
		easyService = easyServiceService.getEasyServiceById(id);
		request.setAttribute("easyService", easyService);
		return "detail";
	}
	...
	public String editEasyService() throws Exception {
		String id = request.getParameter("id");
		easyService = easyServiceService.getEasyServiceById(id);
		request.setAttribute("easyService", easyService);
		return "edit";
	...
	public String delEasyServiceById() throws IOException {
		String id = request.getParameter("id");
		try {
			easyService.setId(id);
			easyServiceService.delEasyService(easyService);
		} catch (Exception e) {
	...
	public String delEasyServiceByIds() throws IOException {
		...
		try {
			...
			easyServiceService.delEasyServiceBatch(idList);
		} catch (Exception e) {
	...
	public String saveEasyService() throws Exception {
		easyServiceService.saveEasyService(easyService);
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/easyservice/searchEasyService.do");
		return "success";
	}
	...
	public String searchEasyService() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("easyServiceList").		//easyServiceList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(!StringUtil.null2String(easyService.getName()).equals("")) {
				sqlWhere.append(" and easyService.name='" + easyService.getName() + "'");
			}
			request.setAttribute("easyService", easyService);
		} else {
			request.setAttribute("easyService", null);
		}
		...
		Map map = easyServiceService.searchEasyService(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openEasyServiceExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = easyService.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and easyService.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<EasyService> list = easyServiceService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(EasyService easyService : list) {
				Object[] tempModel = new Object[ja.size()];									//临时的model
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = easyService.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(easyService, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openEasyServiceImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, EasyService.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<EasyService> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					easyServiceService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/easyservice/searchEasyService.do");
	 ...
	 */
	@Security
	public String saveEasyService4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 		easyService = easyServiceService.saveEasyService(easyService);
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(easyService).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editEasyService4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delEasyServiceByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		easyServiceService.delEasyServiceBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchEasyService4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
