package com.cityinspector.viewmesasge.action;
...
/**
 * @author 王富生
 * 创建时间：2018-07-22 15:11:55
 */
@SuppressWarnings("finally")
@Controller
public class ViewMessageAction extends BaseAction implements ModelDriven<Object>{

	private Long PAGESIZE_CONSTANT = 10l;//显示记录数

	@Resource
	private IViewMessageService viewMessageService;

	/**
	 * 将form转成model
	 */
	private ViewMessage viewMessage;
	public ViewMessage getModel() {
		if(viewMessage == null) {
	...
	public String addViewMessage() throws Exception {
		return "edit";
	}
	...
	public String detailViewMessage() throws Exception {
		String id = request.getParameter("id");
		viewMessage = viewMessageService.getViewMessageById(id);
	...
	public String editViewMessage() throws Exception {
		String id = request.getParameter("id");
		viewMessage = viewMessageService.getViewMessageById(id);
		request.setAttribute("viewMessage", viewMessage);
		return "edit";
	...
	public String delViewMessageById() throws IOException {
		String id = request.getParameter("id");
		try {
			viewMessage.setId(id);
			viewMessageService.delViewMessage(viewMessage);
		} catch (Exception e) {
	...
	public String delViewMessageByIds() throws IOException {
		...
		try {
			...
			viewMessageService.delViewMessageBatch(idList);
		} catch (Exception e) {
	...
	public String saveViewMessage() throws Exception {
		viewMessageService.saveViewMessage(viewMessage);
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/viewmesasge/searchViewMessage.do");
		return "success";
	}
	...
	public String searchViewMessage() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("viewMessageList").		//viewMessageList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(!StringUtil.null2String(viewMessage.getTitle()).equals("")) {
				sqlWhere.append(" and viewMessage.title='" + viewMessage.getTitle() + "'");
			}
			request.setAttribute("viewMessage", viewMessage);
		} else {
			request.setAttribute("viewMessage", null);
		}
		...
		Map map = viewMessageService.searchViewMessage(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openViewMessageExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = viewMessage.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and viewMessage.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<ViewMessage> list = viewMessageService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(ViewMessage viewMessage : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = viewMessage.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(viewMessage, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openViewMessageImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, ViewMessage.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<ViewMessage> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					viewMessageService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/viewmesasge/searchViewMessage.do");
	 ...
	 */
	@Security
	public String saveViewMessage4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 		viewMessage = viewMessageService.saveViewMessage(viewMessage);
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(viewMessage).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editViewMessage4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delViewMessageByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		viewMessageService.delViewMessageBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchViewMessage4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
