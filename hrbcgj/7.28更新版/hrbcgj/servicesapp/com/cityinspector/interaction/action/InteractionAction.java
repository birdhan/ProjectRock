package com.cityinspector.interaction.action;
...
/**
 * @author 王富生
 * 创建时间：2018-07-21 14:10:47
 */
@SuppressWarnings("finally")
@Controller
public class InteractionAction extends BaseAction implements ModelDriven<Object>{
	...
	public String addInteraction() throws Exception {
		return "edit";
	}
	...
	public String detailInteraction() throws Exception {
		String id = request.getParameter("id");
		interaction = interactionService.getInteractionById(id);
	...
	public String editInteraction() throws Exception {
		String id = request.getParameter("id");
		interaction = interactionService.getInteractionById(id);
		request.setAttribute("interaction", interaction);
		return "edit";
	...
	public String delInteractionById() throws IOException {
		String id = request.getParameter("id");
		try {
			interaction.setId(id);
			interactionService.delInteraction(interaction);
		} catch (Exception e) {
	...
	public String delInteractionByIds() throws IOException {
		...
		try {
			...
			interactionService.delInteractionBatch(idList);
		} catch (Exception e) {
	...
	public String saveInteraction() throws Exception {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/interaction/searchInteraction.do");
		return "success";
	}
	...
	public String searchInteraction() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("interactionList").		//interactionList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(!StringUtil.null2String(interaction.getTitle()).equals("")) {
				sqlWhere.append(" and interaction.title='" + interaction.getTitle() + "'");
			}
			if(!StringUtil.null2String(interaction.getReqregisteruser()).equals("")) {
				sqlWhere.append(" and interaction.reqregisteruser='" + interaction.getReqregisteruser() + "'");
			}
			if(!StringUtil.null2String(interaction.getRepstatus()).equals("")) {
				sqlWhere.append(" and interaction.repstatus='" + interaction.getRepstatus() + "'");
			}
			request.setAttribute("interaction", interaction);
		} else {
			request.setAttribute("interaction", null);
		}
		...
		Map map = interactionService.searchInteraction(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openInteractionExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = interaction.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and interaction.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<Interaction> list = interactionService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(Interaction interaction : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = interaction.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(interaction, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openInteractionImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, Interaction.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<Interaction> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					interactionService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/interaction/searchInteraction.do");
	 ...
	 */
	@Security
	public String saveInteraction4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 		interaction = interactionService.saveInteraction(interaction);
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(interaction).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editInteraction4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delInteractionByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		interactionService.delInteractionBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchInteraction4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
