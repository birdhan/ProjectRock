package com.cloud.requestrecord.action;
...
import com.cloud.requestrecord.service.IRequestRecordService;
import com.opensymphony.xwork2.ModelDriven;

/**
 * @author cuiyp
 * 创建时间：2018-07-22 15:39:42
 */
@SuppressWarnings("finally")
@Controller
public class RequestRecordAction extends BaseAction implements ModelDriven<Object>{
	...
	public String addRequestRecord() throws Exception {
		return "edit";
	}
	...
	public String detailRequestRecord() throws Exception {
		String id = request.getParameter("id");
		requestRecord = requestRecordService.getRequestRecordById(id);
		request.setAttribute("requestRecord", requestRecord);
		return "detail";
	}
	...
	public String editRequestRecord() throws Exception {
		String id = request.getParameter("id");
		requestRecord = requestRecordService.getRequestRecordById(id);
		request.setAttribute("requestRecord", requestRecord);
		return "edit";
	...
	public String delRequestRecordById() throws IOException {
		String id = request.getParameter("id");
		try {
			requestRecord.setId(id);
			requestRecordService.delRequestRecord(requestRecord);
		} catch (Exception e) {
	...
	public String delRequestRecordByIds() throws IOException {
		...
		try {
			...
			requestRecordService.delRequestRecordBatch(idList);
		} catch (Exception e) {
	...
	public String saveRequestRecord() throws Exception {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/requestrecord/searchRequestRecord.do");
		return "success";
	}
	...
	public String searchRequestRecord() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("requestRecordList").		//requestRecordList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(!StringUtil.null2String(requestRecord.getUseraccount()).equals("")) {
				sqlWhere.append(" and requestRecord.useraccount='" + requestRecord.getUseraccount() + "'");
			}
			if(!StringUtil.null2String(requestRecord.getFormip()).equals("")) {
				sqlWhere.append(" and requestRecord.formip='" + requestRecord.getFormip() + "'");
			}
			request.setAttribute("requestRecord", requestRecord);
		} else {
			request.setAttribute("requestRecord", null);
		}
		...
		Map map = requestRecordService.searchRequestRecord(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openRequestRecordExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = requestRecord.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and requestRecord.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<RequestRecord> list = requestRecordService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(RequestRecord requestRecord : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = requestRecord.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(requestRecord, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openRequestRecordImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, RequestRecord.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<RequestRecord> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					requestRecordService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/requestrecord/searchRequestRecord.do");
	 ...
	 */
	@Security
	public String saveRequestRecord4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 		requestRecord = requestRecordService.saveRequestRecord(requestRecord);
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(requestRecord).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editRequestRecord4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delRequestRecordByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		requestRecordService.delRequestRecordBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchRequestRecord4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
