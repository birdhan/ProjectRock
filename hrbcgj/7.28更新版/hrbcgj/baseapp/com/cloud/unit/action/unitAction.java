...
import com.cloud.unit.service.IunitService;
import com.opensymphony.xwork2.ModelDriven;

/**
 * @author 王子健
 * 创建时间：2018-07-22 21:29:34
 */
@SuppressWarnings("finally")
@Controller
public class unitAction extends BaseAction implements ModelDriven<Object>{
	...
	public String detailunit() throws Exception {
	...
	public String delunitById() throws IOException {
		String id = request.getParameter("id");
		try {
			unit.setId(id);
	...
	public String delunitByIds() throws IOException {
		...
		try {
			...
			unitService.delunitBatch(idList);
		} catch (Exception e) {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/unit/searchunit.do");
		return "success";
	}
	...
	public String searchunit() throws Exception {
		...
		String pageIndexName = new org.displaytag.util.ParamEncoder("unitList").		//unitList为jsp页面中<display:table> 的id值
		...
		Map map = unitService.searchunit(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openunitExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = unit.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and unit.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<unit> list = unitService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(unit unit : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = unit.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(unit, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openunitImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, unit.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<unit> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					unitService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/unit/searchunit.do");
	 ...
	 */
	@Security
	public String saveunit4Json() throws Exception {
	 	...
	 	try {
	 		...
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(unit).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editunit4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delunitByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		unitService.delunitBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchunit4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
