package com.cloud.special.action;
...
/**
 * @author 王子健
 * 创建时间：2018-07-22 16:32:13
 */
@SuppressWarnings("finally")
@Controller
public class specialAction extends BaseAction implements ModelDriven<Object>{
	...
	public String addspecial() throws Exception {
		return "edit";
	}
	...
	public String detailspecial() throws Exception {
		String id = request.getParameter("id");
		special = specialService.getspecialById(id);
	...
	public String editspecial() throws Exception {
		String id = request.getParameter("id");
		special = specialService.getspecialById(id);
		request.setAttribute("special", special);
		return "edit";
	...
	public String delspecialById() throws IOException {
		String id = request.getParameter("id");
		try {
			special.setId(id);
			specialService.delspecial(special);
		} catch (Exception e) {
	...
	public String delspecialByIds() throws IOException {
		...
		try {
			...
			specialService.delspecialBatch(idList);
		} catch (Exception e) {
	...
	public String savespecial() throws Exception {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/special/searchspecial.do");
		return "success";
	}
	...
	public String searchspecial() throws Exception {
		...
		String pageIndexName = new org.displaytag.util.ParamEncoder("specialList").		//specialList为jsp页面中<display:table> 的id值
		...
		if(isSearch.equals("yes")) {													//表示是查询
			...
			request.setAttribute("special", null);
		}
		...
		Map map = specialService.searchspecial(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openspecialExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = special.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and special.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<special> list = specialService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(special special : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = special.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(special, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openspecialImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, special.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<special> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					specialService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/special/searchspecial.do");
	 ...
	 */
	@Security
	public String savespecial4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 		special = specialService.savespecial(special);
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(special).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editspecial4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delspecialByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		specialService.delspecialBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchspecial4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
