...
<body>
	...
	<div id="cgdt_sy">
		...
		<div
			style="width:240px;height:60px;background-color:#1fc1dd ;margin-left: 50px;margin-top: 40px;font:24px '微软雅黑';color:#fff;line-height: 60px;text-align: center;">我的信箱</div>
		<div
			...
			<a href="javascript:void(0);" onclick="writeEmail()"
				style="font:24px '微软雅黑';color:#fff;line-height: 30px;">我要写信</a>
		</div>
		
		<form id="myEmailForm" action="${ctx}/myEmail?sectionId=${sectionId}&secondSectionId=${secondSectionId}" method="post">
		...
		<table style="margin:0 auto;">
			<tr>
				...
				<td style="font: 20px '微软雅黑';color: #000;padding:40px 80px 20px 80px;border:none;">申请日期</td>
			</tr>
			
			<c:forEach items="${emailList }" var="email" varStatus="status">
				<tr>
					<td style="padding:0px 0px 0px 0px;border:none;text-align: left;">
						<a href="javascript:void(0);" onclick="emailDetail('${email.ID}')" style="font: 14px '微软雅黑';color: #000;line-height: 30px;">
							<c:choose>
								<c:when test="${fn:length(email.TITLE)>25}">
									${fn:substring(email.TITLE, 0, 25)  }...
								</c:when>
								<c:otherwise>
									${email.TITLE}
								</c:otherwise>
					...
					<td style="font: 14px '微软雅黑';color: #000;padding:0px 60px 0px 60px;border:none;text-align: center;line-height: 30px;">
						<c:choose>
							<c:when test="${email.REPSTATUS == '0' }">
								未回复
							</c:when>
							<c:when test="${email.REPSTATUS == '1' }">
								已回复
							</c:when>
							<c:otherwise>
								已阅读
					...
					<td
						...
						（<fmt:formatDate value="${email.REQTIME }" pattern="yyyy-MM-dd" />）
					</td>
				</tr>
			</c:forEach>
		</table>
			...
			<table style="width:100%;">
				<tr>
					<td style="text-align: right;">
						...
						<cloud:breakPage pageSize="${resultSize}" url="${ctx}/myEmail?sectionId=${sectionId}&secondSectionId=${secondSectionId}" curPage="${curPage}" formId="myEmailForm" totalNum="${pageSize}"/>
					</td>
				</tr>
			  </table>
		</div>
...
function writeEmail(){
	...
	window.location.href = "${ctx}/writeEmail?sectionId=index&secondSectionId=default&usercuid="+loginUserCuid;
}

function emailDetail(letterId){
	var sessionUserId = "${sessionScope.webSiteLoginUser.account}";
	if(sessionUserId == "" || sessionUserId == null){
		$('#login_background').css('display', 'block');
		$('#login_alert').css('display', 'block');
	...
	window.open("${ctx}/emailDetail?sectionId=index&secondSectionId=default&letterId="+letterId);
}
</script>
</html>
