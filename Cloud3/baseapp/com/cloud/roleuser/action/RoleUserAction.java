package com.cloud.roleuser.action;
...
/**
 * @author cloudwork
 * 创建时间：2013-12-10 13:23:35
 */
@SuppressWarnings("finally")
@Controller
public class RoleUserAction extends BaseAction implements ModelDriven<Object>{
	...
	public String detailRoleUser() throws Exception {
		String id = request.getParameter("id");
		roleUser = roleUserService.getRoleUserById(id);
	...
	public String editRoleUser() throws Exception {
		String id = request.getParameter("id");
		roleUser = roleUserService.getRoleUserById(id);
		request.setAttribute("roleUser", roleUser);
		return "edit";
	...
	public String delRoleUserById() throws IOException {
		String id = request.getParameter("id");
		try {
			roleUser.setId(id);
			roleUserService.delRoleUser(roleUser);
		} catch (Exception e) {
	...
	public String delRoleUserByIds() throws IOException {
		...
		try {
			...
			roleUserService.delRoleUserBatch(idList);
		} catch (Exception e) {
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/roleuser/searchRoleUser.do");
		return "success";
	}
	...
	public String searchRoleUser() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("roleUserList").		//roleUserList为jsp页面中<display:table> 的id值
		...
		if(isSearch.equals("yes")) {													//表示是查询
			...
			request.setAttribute("roleUser", null);
		}
		...
		Map map = roleUserService.searchRoleUser(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openRoleUserExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = roleUser.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and roleUser.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<RoleUser> list = roleUserService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(RoleUser roleUser : list) {
				...
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = roleUser.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(roleUser, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openRoleUserImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, RoleUser.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<RoleUser> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					roleUserService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/roleuser/searchRoleUser.do");
	 ...
	 */
	@Security
	 public String saveRoleUser4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 		jsonData = StringUtil.null2String(request.getParameter("jsonData"));
	 		roleUser = (RoleUser)JSONUtil.json2Object(jsonData, RoleUser.class.getName());
	 		roleUser = roleUserService.saveRoleUser(roleUser);
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(roleUser).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	 public String editRoleUser4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	 public String delRoleUserByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		roleUserService.delRoleUserBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	 public String searchRoleUser4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	  ...
	  */
	@Security
	 public String saveRoleUserId() throws Exception {
		 String roleId = request.getParameter("roleId");				//关联角色id
		 String userId = request.getParameter("userId");				//用户账号逗号分隔
		 boolean flag = false;
		 List<RoleUser> list = new ArrayList();
		 if(!roleId.equals("") && !userId.equals("")) {
			 for(String uid : userId.split(",")) {
				 RoleUser ru = new RoleUser();
				 ru.setLinkRoleId(roleId);
				 ru.setUserId(uid);
				 list.add(ru);
			 } 
		 }		 
		 if(list.size() != 0) {
			 roleUserService.delRoleUserByRoleId(roleId);				//先删除其关系
			 flag = roleUserService.saveDataBatch(list);
		 }		 
