...
(function ($)
{
    ...
    $.ligerDefaults.Grid.formatters['date'] = function (value, column)
    {
        ...
        if(typeof (value) == "object") {	//如果是object类型
        	var dateData = new Date(value.time); 	//得到1328423451489串转成date对象
        	return dateData.format(column.format);
        }
        else
        {
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' size='4' id='cur_page' value='1' style='width:20px' maxlength='3' /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
        ...
        */
        ...
        _buildPager: function (type)
        {
            ...
            if(type != "create") {						//只要不是初始创建的，在后期操作刷新按钮就判断是否存在数据
            	if(p.total == 0) {
                	if(!tip) {
                		f_tip("<font color='red'>当前页中已没有数据！</font>");
                	}            	
                }
        	}            
        },
