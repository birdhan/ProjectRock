		...
		try {
			...
			if(map.size() != 0 && map.get("dictvalue") != null && map.get("dictlabel") != null) {
				...
				for(int i=0;i<tv_arr.length;i++) {
					...
					sb.append("<div style=\"width:20px;width:auto;overflow:hidden;\"><span class='span_1'><input type=\"checkbox\"  " + checked + " " + StringUtil.null2String(style) + " id=\"" +property+i+ "\" name=\"" + property + "\" value=\""+tv_arr[i].trim()+"\"/></span><span class='span_2'><label for=\"" +property+i+ "\">" + tl_arr[i].trim() + "</label></span></div>");
				}
