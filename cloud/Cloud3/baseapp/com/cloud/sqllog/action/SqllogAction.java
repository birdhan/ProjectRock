package com.cloud.sqllog.action;
...
import com.cloud.sqllog.service.ISqllogService;
import com.opensymphony.xwork2.ModelDriven;

/**
 * @author cloudwork
 * 创建时间：2014-01-06 12:56:58
 */
@SuppressWarnings("finally")
@Controller
public class SqllogAction extends BaseAction implements ModelDriven<Object>{
	...
	public String addSqllog() throws Exception {
		return "edit";
	}
	...
	public String detailSqllog() throws Exception {
		String id = request.getParameter("id");
		sqllog = sqllogService.getSqllogById(id);
		request.setAttribute("sqllog", sqllog);
		return "detail";
	}
	...
	public String editSqllog() throws Exception {
		String id = request.getParameter("id");
		sqllog = sqllogService.getSqllogById(id);
		request.setAttribute("sqllog", sqllog);
		return "edit";
	...
	public String delSqllogById() throws IOException {
		String id = request.getParameter("id");
		try {
			sqllog.setId(id);
			sqllogService.delSqllog(sqllog);
		} catch (Exception e) {
	...
	public String delSqllogByIds() throws IOException {
		...
		try {
			...
			sqllogService.delSqllogBatch(idList);
		} catch (Exception e) {
	...
	public String saveSqllog() throws Exception {
		sqllogService.saveSqllog(sqllog);
	...
	public String success() throws Exception {
		request.setAttribute("listUrl", request.getContextPath() + "/sqllog/searchSqllog.do");
		return "success";
	}
	...
	public String searchSqllog() throws Exception {
		StringBuffer sqlWhere = new StringBuffer("");										//查询条件where
		String pageIndexName = new org.displaytag.util.ParamEncoder("sqllogList").		//sqllogList为jsp页面中<display:table> 的id值
				encodeParameterName(org.displaytag.tags.TableTagParameters.PARAMETER_PAGE);

		//查询过滤-start
		String isSearch = StringUtil.null2String(request.getParameter("isSearch"));		//查询标志
		if(isSearch.equals("yes")) {													//表示是查询
			if(sqllog.getFbegintimeSearch() != null) {
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				sqlWhere.append(" and sqllog.fbegintime >="+DBFM.TO_DATE()+"('"+sdf.format(sqllog.getFbegintimeSearch())+"','yyyy-MM-dd HH24:mi:ss')");
			}
			if(sqllog.getFendtimeSearch() != null) {
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				sqlWhere.append(" and sqllog.fbegintime <="+DBFM.TO_DATE()+"('"+sdf.format(sqllog.getFendtimeSearch())+"','yyyy-MM-dd HH24:mi:ss')");
			}
			if(sqllog.getFsqltype() != 0) {
				sqlWhere.append(" and sqllog.fsqltype=" + sqllog.getFsqltype());
			}
				
			request.setAttribute("sqllog", sqllog);
		} else {
			request.setAttribute("sqllog", null);
		}
		sqlWhere.append(" order by sqllog.fbegintime desc");
		//查询过滤-end
		...
		Map map = sqllogService.searchSqllog(pageIndex,PAGESIZE_CONSTANT,sqlWhere.toString());
	...
	public String openSqllogExport() throws Exception {
		String ids = request.getParameter("ids");
		Map fieldMap = new TreeMap();
		Field[] fieldArr = sqllog.getClass().getDeclaredFields();							//得到定义的属性
	...
	public String exportData() throws Exception {
		...
		if(!ids.equals("")) {
			...
			if(!inIds.equals("")) {
				...
				where.append(" and sqllog.id in (" + inIds + ")");
			}
		}
		...
		if(!StringUtil.null2String(columns).equals("")) {
			...
			List<Sqllog> list = sqllogService.getAllDataByWhere(where.toString());			//得到要导出的数据
			...
			for(Sqllog sqllog : list) {
				Object[] tempModel = new Object[ja.size()];									//临时的model
				for(int i=0;i<ja.size();i++) {												//遍历所有选中的列内容
					JSONObject jo = (JSONObject)ja.get(i);
					Method m = sqllog.getClass().getMethod("get"+StringUtil.replaceFirstStr2UpperCase(String.valueOf(jo.get("value"))), null);		//得到方法
					Object obj = m.invoke(sqllog, null);								//调用方法后的返回值
					if(obj != null) {
	...
	public String openSqllogImport() throws Exception {
		return "import";
	}
	...
	public String importData() throws Exception {
		...
		Object[] obj = ExcelMap2Model.excleMap2Model(excelMap, Sqllog.class);
		if(obj != null) {
			boolean flag = (Boolean)obj[0];														//验证标志：true为正确
			if(flag) {																			//表示flag值为true
				List<Sqllog> saveList = (List)obj[1];									//取出保存对象
				if(saveList.size() != 0) {
					sqllogService.saveDataBatch(saveList);										//批量保存
		...
		request.getSession().setAttribute("returnUrl", request.getContextPath()+"/sqllog/searchSqllog.do");
	 ...
	 */
	@Security
	public String saveSqllog4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 		jsonData = StringUtil.null2String(request.getParameter("jsonData"));
	 		sqllog = (Sqllog)JSONUtil.json2Object(jsonData, Sqllog.class.getName());
	 		sqllog = sqllogService.saveSqllog(sqllog);
	 		code = 1;
	 		jsonData = JSONUtil.object2JSONObject(sqllog).toString();
	 	} catch (Exception e) {
	 		e.printStackTrace();
	 ...
	 */
	@Security
	public String editSqllog4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	try {
	 ...
	 */
	@Security
	public String delSqllogByIds4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	 	try {
	 		...
	 		sqllogService.delSqllogBatch(idList);												//批量删除
	 		code = 1;																		//返回代码
	 		jsonData = "删除成功";															//返回信息
	 	} catch (Exception e) {
	 		String mesg = "批量删除数据错误，消息为："+e.getMessage();
	 ...
	 */
	@Security
	public String searchSqllog4Json() throws Exception {
	 	JSONObject root = new JSONObject();
	 	int code = 0;																		//默认失败
	 	String jsonData = "";
	 	String ids = request.getParameter("ids");
	...
	@Security
	public String clearSqllog() throws Exception {
		String sql = "delete from T_LOG_SQLLOG";
