...
<link rel="stylesheet"
	...
	type="text/css">
<script type="text/javascript" src="${pageContext.request.contextPath }/js/indexreception.js"></script>
...
function han(dome,id,danjia){
	alert("修改成功");
		window.location.href="${pageContext.request.contextPath}/byIdupnumber?id="+id+"&number="+dome+"&spay="+danjia;
	
		
	
}

function payorder(id,numpay){
	
	if(<%=session.getAttribute("monery")%>>=numpay){
		if(confirm("确认花费"+numpay+"元购买此商品")){
			window.location.href="${pageContext.request.contextPath}/payorder?id="+id+"&payyuan="+numpay;
			}
	}else{
		alert("余额不足请联系管理员充值");
	}
					...
					<div class="col-md-4" style="padding-top: 5px;">
						<p>
							购买数量：<input type="text" name="number"
								onkeyup="value=value.replace(/[^\d]/g,'') " value="${listshop.snumber }"
								style="width: 140px;" onchange="han(this.value,'${listshop.id }','${listshop.kpic }')">
						</p>
						<p><span>订单总价：</span>$<span>${listshop.spay }</span></p>
						<p style="float: right;">
							<button class="btn btn-success" onclick="payorder('${listshop.id }','${listshop.spay }')">立即付款</button>
						</p>
					</div>
				</div>
			</div>
